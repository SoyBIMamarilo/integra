...
//     return projects.map(project => createTreeNode(`project|${hubId}|${project.id}`, project.attributes.name, project.attributes.extension.data.projectType == "BIM360" ? 'icon-project-bim360' : 'icon-project-acc', true));
// }
...
const SidebarAps = () => {
    ...
    const [sidebar3D, setsidebar3D] = useState(null);
    const sidebar3DRef = useRef(null);
    const handlerOnchangedTree = () => { console.log("se realizo un click") }
    const initTree = (selector, onSelectionChanged) => {
        const tree = new InspireTree({
            data: function (node) {
                if (!node || !node.id) {
                    return getHubs();
    ...
    initTree(sidebar3DRef.current, handlerOnchangedTree).catch((err) => { console.log(err); })
    return (
        <>
            <div id="sidebar">
