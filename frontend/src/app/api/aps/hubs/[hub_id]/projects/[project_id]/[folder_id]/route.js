...
export async function GET(request, { params }) {
    try {
        ...
        if (cookieAps) {
            ...
            let resp = "";
            if (!params.folder_id) {
                resp = await new APS.ProjectsApi().getProjectTopFolders(params.hub_id, params.project_id, internalAuthClient, internalOAuthToken);
            } else {
                resp = await new APS.FoldersApi().getFolderContents(params.project_id, params.folder_id, null, internalAuthClient, internalOAuthToken);
            }
