...
export async function GET(req, res) {
  try {
    ...
    let { data: data_item_manual, error_manual } = await supabase.rpc(
      "presupuesto_por_item_manual",
      { presupuesto: budget_id }
    );
    let { data: data_item, error_item } = await supabase.rpc(
      "presupuesto_por_item",
      { presupuesto: budget_id }
    );
    data_item_manual = data_item_manual.map((item) => ({
      ...item,
      type: "manual",
      pyrefnombre: "",
      cbs: "",
